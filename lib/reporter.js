...
var generateReport = function (options) {
    ...
    var readFileForRespectiveTemplates = function (filename) {
        ...
        if ((filename === 'features.html' || filename === 'piechart.js') && options.theme === 'bootstrap.bundle') {
            return readFile('../_common/bootstrap.hierarchy/' + filename)
        }
    ...
    // for bootstrap.bundle, also copy fonts
    if (options.theme === 'bootstrap.bundle') {
        const dest = path.join(path.dirname(options.output), 'fonts')
        if (!fs.existsSync(dest)) {
            fs.mkdirSync(dest);
        }
        ['fonts/glyphicons-halflings-regular.woff2', 'fonts/glyphicons-halflings-regular.woff', 'fonts/glyphicons-halflings-regular.ttf'].forEach((f) => {
            fs.copyFile(`${path.join(__dirname, '..', 'templates/bootstrap.bundle')}/${f}`, path.join(path.dirname(options.output), f),(err) => {
                if (err) throw err;
            });
        })
    }
    ...
    fs.writeFileSync(
        ...
        _.template(readFile('index.html'))({
            ...
            piechart: ((options.theme === 'bootstrap') || (options.theme === 'hierarchy') || (options.theme === 'bootstrap.bundle')) ? readFileForRespectiveTemplates('piechart.js') : undefined,
            guid: guid
        })
    );
